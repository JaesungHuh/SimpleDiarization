...
class DiarizationModule():
    ...
    def run(self, wav_file, ref_rttm_file, vad_file=""):
        ...
        if vad_file == "" or (os.path.isfile(vad_file) == False):
            orig_vadresults = self.vad_module.get_pyannote_segments(
                wav_file)
        else:
        ...
        if self.cfg.vad.merge_vad:
            # You have to use the original start and end time from vad file
            ...
            # Extract embeddings
            embeddings = self.embedding_module.extract_embeddings(
                wav_file, vad_segments)
